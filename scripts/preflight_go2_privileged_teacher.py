"""No-learning GPU preflight for one Go2 privileged-teacher arm."""
...
def run_preflight(
  *, arm: str, manifest: Path, manifest_sha256: str, num_envs: int,
  steps: int, device: str, allow_env_state_mismatch: bool,
) -> dict[str, Any]:
  if arm not in TASK_IDS or num_envs <= 0 or steps <= 0:
    raise ValueError("invalid arm, num_envs, or steps")
  if allow_env_state_mismatch == (num_envs == 2048):
    raise ValueError(
      "environment-state mismatch override is required only for non-2048 "
      "technical smoke runs"
    )
  ...
  try:
    ...
    if runner_cls is None:
      raise RuntimeError("privileged teacher task has no registered runner")
    runner = runner_cls(wrapped, asdict(agent_cfg), device=device)
    transfer = runner.load(str(SOURCE_CHECKPOINT), strict=True, map_location=device)
    if bool(transfer["environment_state_restored_exact"]) is not (
      num_envs == 2048
    ):
      raise RuntimeError("environment restoration mode differs from env count")
    if num_envs == 2048:
      state = {
    ...
    if actor_dim == CANDIDATE_ACTOR_DIM:
      ...
      first_layer_zero = True
      normalizer_exact = True
    if not first_layer_zero or not normalizer_exact:
    ...
    if actor_observation.shape != (num_envs, actor_dim):
      raise RuntimeError(
        f"unexpected actor observation shape: {actor_observation.shape}"
      )
    if critic_observation.shape != (num_envs, CRITIC_DIM):
      raise RuntimeError(
        f"unexpected critic observation shape: {critic_observation.shape}"
      )
    policy = runner.get_inference_policy(device=device)
    with torch.inference_mode():
      initial_action = policy(observation)
      initial_critic = runner.alg.critic(observation)
    initial_prefix = actor_observation[:, :SOURCE_ACTOR_DIM]
    finite = bool(
    ...
    return {
      ...
      "candidate_new_columns_zero": first_layer_zero,
      "candidate_normalizer_exact": normalizer_exact,
      "initial_actor_prefix_sha256": _tensor_sha(initial_prefix),
      "initial_deterministic_action_sha256": _tensor_sha(initial_action),
      "initial_action_abs_max": float(initial_action.abs().max()),
      "finite": {"observations_actions": finite, "rewards": rewards_finite},
      "reset_count": resets,
      "formal_environment_state_exact": bool(
        transfer["environment_state_restored_exact"]
      ),
      "technical_smoke_override": bool(
        transfer["technical_smoke_env_state_override"]
      ),
    }
  finally:
    env.close()
...
def main() -> None:
  ...
  parser.add_argument("--allow-env-state-mismatch", action="store_true")
  parser.add_argument("--output-file", type=Path, required=True)
  args = parser.parse_args()
  payload = run_preflight(
    arm=args.arm, manifest=args.source_manifest,
    manifest_sha256=args.source_manifest_sha256, num_envs=args.num_envs,
    steps=args.steps, device=args.device,
    allow_env_state_mismatch=args.allow_env_state_mismatch,
  )
